<?php
//inspiration : https://medium.com/@selieshjksofficial/creating-and-managing-jwt-tokens-in-php-b6c1fc6c1b46 
...
class JwtManager
{
    ...
    public function expiratedToken($token)
    {
        list(, $base64UrlPayload,) = explode('.', $token);
        $payload = $this->base64UrlDecode($base64UrlPayload);
        return time() - json_decode($payload, true)["date"] > self::SECONDS_IN_HOUR; // expire au bout d'une heure
    }
